...
async function post({
  ...
  await Firestore.runTransaction(async (transaction) => {
    ...
    if (memberDoc.exists === false) {
      throw new CustomServerError({ statusCode: 400, message: '존재하지 않는 사용자에게 질문을 보내고 있어요 😫' });
    }
...
async function postReply({ uid, messageId, reply }: { uid: string; messageId: string; reply: string }) {
  ...
  await Firestore.runTransaction(async (transaction) => {
    ...
    if (messageData.reply !== undefined) {
      throw new CustomServerError({ statusCode: 400, message: '이미 답변이 달린 질문에 답변을 보내고 있어요 😫' });
    }
